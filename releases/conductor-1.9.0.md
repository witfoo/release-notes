...
- The destination is pinned to the CrowdStrike ingest domains over HTTPS with certificate verification, and redirects are re-checked against the same rule, so an exporter can never be steered to another host. A rejected API key is surfaced as a connector failure in Integration Health rather than silently retried; transient ingest errors keep the connector up and redeliver the batch.
- **All exporters:** predicate filters configured on an exporter now evaluate correctly; previously a filter on an exporter could reject every artifact.
...
- A new **`recon`** message type, a new **"Web Reconnaissance Detected"** lead rule (rule 330, Recon behavioral sets) and a new **"Web Reconnaissance"** work-unit definition (MO 10) keep these probes visible as work units under the correct category, with the probing client IP and requested path extracted onto the artifact. Both are added to existing appliances automatically at the next incident-engine start (inserted only if absent; operator edits are never overwritten).
- Genuine service failures — upstream and PHP-FPM connection failures, timeouts, TLS handshake failures, missing server-side files such as a `.htpasswd`, 5xx responses — continue to classify as `degraded_service`: a missing-file error counts as a probe only when the file nginx failed to open is the request's own target. Ordinary access-log records are unchanged.
...
- **Lead rules and work-unit definitions.** The new "Web Reconnaissance Detected" rule and "Web Reconnaissance" definition are seeded on fresh installs, delivered to licensed appliances through the Intel metadata sync, and reconciled onto every existing appliance at incident-engine start — no manual step.
